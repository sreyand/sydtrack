...
assert(/#view-settings #screen-limit-row \{[\s\S]{0,50}border-bottom:\s*0/.test(homeCss), 'Settings screen-time row does not double the next section divider');
...
assert(rendererJs.includes('fmtDuration(total)'), 'Home donut uses the pre-#25 duration format');
assert(!rendererJs.includes('fmtPieDuration'), 'Home donut no longer uses the post-#25 pie duration helper');
