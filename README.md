...
Settings can export JSON (`.sydtrack`, format `sydtrack-backup`, backup schema 1) or CSV (one spreadsheet of activity, sessions, settings, tags, and profiles). Both are local files. Older `focusflow-backup` and `focusflow-profile` files still import. **Delete all my data** removes activity, rollups, sessions, settings, profiles, tags, logs, and migration copies on this machine and restores the bundled defaults. **Clear all history** removes activity only.
